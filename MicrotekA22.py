...
MAINFOLDER = r"D:\APPdata_MicrotekA22"

# Create other paths relative to the main folder
training_file_path = os.path.join(MAINFOLDER, "Training", "Training_MicrotekA22.xlsx")  # FIXED TRAINING DATA
test_file_path = os.path.join(MAINFOLDER, "24hrData", "Dailydata_MicrotekA22.xlsx")  # DAILY DATA
excel_file_path = os.path.join(MAINFOLDER, "Breakdownrecords_MicrotekA22.xlsx")  # Recording excel for BD
folderpath = os.path.join(MAINFOLDER, "TemporaryData")  # Temporary dump files collector
threshold_file_path = os.path.join(MAINFOLDER,"Thresholds_MicrotekA22.xlsx") #
...
def process_data():
    ...
    assets_list = [
        "Micotek Burnising Machine MDE","Micotek Burnising Machine MNDE","Micotek Burnising Machine Spindle DE",
        "Micotek Burnising Machine Spindle NDE"]
...
def train_ensemble_model(training_file_path, model_folder_path):
    ...
    def preprocess_data(X, y):
        ...
        # Save the scaler
        joblib.dump(scaler, os.path.join(model_folder_path, "scaler_MicrotekA22.pkl"))
    ...
    joblib.dump(ensemble_model, os.path.join(model_folder_path, "ensemble_MicrotekA22.pkl"))
...
def predict_ensemble(test_file_path, model_folder_path):
    ...
    def preprocess_test_data(X_test):
        scaler = joblib.load(os.path.join(model_folder_path, "scaler_MicrotekA22.pkl"))
        X_test_scaled = scaler.transform(X_test)
        return X_test_scaled

    def predict(X_test_scaled):
        nn_model = joblib.load(os.path.join(model_folder_path, "ensemble_MicrotekA22.pkl"))
...
def train_lstm_autoencoder_model(training_file_path, model_folder_path):
    ...
    # Train the model
    autoencoder.fit(X_seq, X_seq, epochs=100, batch_size=64, shuffle=True)
